/* === 💎 Inspiro AI · v6.3.2 (穩定出圖正式版) ===
   ✅ Pollinations → DeepAI → HuggingFace → MagicStudio 自動備援
   ✅ 修正 DeepAI 表單格式錯誤
   ✅ 強化 buffer 檢查與 HTTPS 圖片路徑
   ✅ 多引擎全員可用 + 管理員免登入
=================================================================== */
...
async function drawWithMagicStudio(prompt) {
  try {
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt + " digital art")}`;
    const r = await fetch(url);
    const buf = Buffer.from(await r.arrayBuffer());
...
app.listen(PORT, "0.0.0.0", () => console.log(`🚀 Inspiro AI v6.3.2 運行中（Port ${PORT}）`));
